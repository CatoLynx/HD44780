...
def run():
    ...
    display = hd44780.Display(backend = hd44780.MCP23017Backend, pinmap = PINMAP, charmap = CHARMAP, lines = 2, columns = 16, skip_init = args.skip_init, debug = False, backend_kwargs = {'port':1, 'address':0x20, 'bank':'A'})
    display.set_display_enable(cursor = args.cursor, cursor_blink = args.cursor_blink)
    display.clear()
    display.home()
    ui = hd44780.DisplayUI(display, hd44780.SystemInput, debug = True)
